...
摘要生成是 NLP 里比较实在的一个任务——把长文章压缩成几句关键信息。传统的 Seq2Seq 模型做抽象式摘要时有两个硬伤：一是容易丢失人名地名这种细节（OOV 问题），二是翻来覆去生成重复内容。Pointer-Generator Networks（指针生成网络）用**指针机制**和**覆盖机制**同时解决了这两个问题。
