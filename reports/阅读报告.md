...
| 训练时间 | 约 5 小时（50k 数据 × 15 epoch） | 约 3 小时（50k 数据 × 3 epoch） |
...
回头看这个实验，虽然从零训练没跑出理想结果，但反而逼我们想了 BART 这个改进方案，也算因祸得福。BART 用 50k 数据和 3 小时就跑到 ROUGE-1=33.60，比从零训练靠谱太多了。这个结果也说明，现在做 NLP 项目，大部分情况下直接上一个预训练模型微调就够了，从零训练的成本和收益确实不成比例。
